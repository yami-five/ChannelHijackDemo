...
static int16_t grid_lerp(int16_t start, int16_t end, uint8_t line,
                         uint8_t linesNum) {
  if (linesNum <= 1u)
    return (int16_t)(start + (((int32_t)end - start) >> 1));

  return (int16_t)(start + (((int32_t)line * ((int32_t)end - start)) /
                            (linesNum - 1u)));
}

static int16_t grid_lerp_position(int16_t start, int16_t end, uint16_t position,
                                  uint16_t span) {
  if (span <= 1u)
    return (int16_t)(start + (((int32_t)end - start) >> 1));

  return (int16_t)(start + (((int32_t)position * ((int32_t)end - start)) /
                            (span - 1u)));
}

static e3d_Point grid_lerp_point(e3d_Point start, e3d_Point end,
...
static uint16_t grid_wrapped_position(uint16_t span, uint8_t line,
                                      uint8_t linesNum, int t,
                                      uint8_t reverse) {
...
static uint8_t grid_clip_out_code(int32_t x, int32_t y, int32_t left,
...
static uint8_t grid_clip_line_to_rect(e3d_Point *start, e3d_Point *end,
...
static void grid_draw_horizontal_line(e3d_EngineContext *engine_ctx,
                                      e3d_Point start, e3d_Point end,
                                      int16_t left, int16_t top, int16_t right,
...
static void draw_grid_values(e3d_EngineContext *engine_ctx, int16_t x,
                             int16_t y, uint16_t height, uint16_t width,
                             uint16_t color, uint8_t linesNum, uint8_t offset,
                             int16_t offsetX, int16_t offsetY, int t,
                             uint8_t direction) {
  ...
  uint8_t moveVertical =
      direction == GRID_DIRECTION_LEFT || direction == GRID_DIRECTION_RIGHT;
  for (uint8_t i = 0; i < linesNum; i++) {
    e3d_Point top;
    e3d_Point bottom;
    if (moveVertical) {
      uint16_t position = grid_wrapped_position(
          width, i, linesNum, t, direction == GRID_DIRECTION_LEFT);
    ...
    if (grid_clip_line_to_rect(&top, &bottom, clipLeft, clipTop, clipRight,
                               clipBottom)) {
  ...
  uint8_t moveHorizontal =
      direction == GRID_DIRECTION_DOWN || direction == GRID_DIRECTION_UP;
  for (uint8_t i = 0; i < linesNum; i++) {
    e3d_Point left;
    e3d_Point right;
    if (moveHorizontal) {
      uint16_t position = grid_wrapped_position(height, i, linesNum, t,
                                                direction == GRID_DIRECTION_UP);
    ...
    if (left.y == right.y) {
      grid_draw_horizontal_line(engine_ctx, left, right, clipLeft, clipTop,
                                clipRight, clipBottom, color);
    } else if (grid_clip_line_to_rect(&left, &right, clipLeft, clipTop,
