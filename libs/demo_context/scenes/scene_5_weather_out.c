...
static void load_assets(e3d_EngineContext *engine_ctx) {
  ...
  channel = engine_ctx->storage->get_sprite(SPRITE_CHANNEL2);
...
static void run_scene(DemoContext *demo_ctx) {
  ...
  if (f < 10u)
    e3d_Painter_DrawSprite(engine_ctx, right_hands[0], rx, (int16_t)(90u - f * 9u), 0, 1);
  else if (f < 30u)
    e3d_Painter_DrawSprite(engine_ctx, right_hands[1], (int16_t)(rx + 1),
                           wave_offset(f, 350u, 0u, 2), 0, 1);
  else if (f < 40u)
    e3d_Painter_DrawSprite(engine_ctx, right_hands[0], rx, (int16_t)((f - 30u) * 9u), 0, 1);
  else
    e3d_Painter_DrawSprite(engine_ctx, right_hands[0], rx,
                           (int16_t)(90 + wave_offset(f, 350u, 0u, 2)), 0, 1);
  uint32_t li = (f > 45u && f <= 55u) ? 1u : 0u;
  e3d_Painter_DrawSprite(engine_ctx, left_hands[li],
