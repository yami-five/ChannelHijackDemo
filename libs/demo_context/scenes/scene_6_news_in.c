...
static void load_assets(e3d_EngineContext *ctx) {
  ...
  assets.agro_main = ctx->storage->get_sprite(19u);
  assets.burak = ctx->storage->get_sprite(23u);
  assets.mouth[0] = ctx->storage->get_sprite(20u);
  assets.mouth[1] = ctx->storage->get_sprite(21u);
  assets.mouth[2] = ctx->storage->get_sprite(22u);
  assets.mouth[3] = ctx->storage->get_sprite(21u);
  assets.segment_zero = ctx->storage->get_sprite(24u);
  assets.segment_three = ctx->storage->get_sprite(27u);
  assets.logo = ctx->storage->get_sprite(52u);
}
...
static void run_scene(DemoContext *demo_ctx) {
  ...
  if ((f >= 5u && f < 10u) || (f >= 30u && f < 35u) ||
    ...
    e3d_Painter_DrawRectangle(ctx, &program_rect, 0x0000);
  } else {
    e3d_Painter_DrawPlasma(ctx, plasma_colors, 16u, f, 2, 6, 6, 7, 6, &plasma_rect);
    e3d_Painter_DrawSprite(ctx, assets.agro_main, 130, 44, 0, 1);
    e3d_Painter_DrawSprite(ctx, assets.burak, 54, 38, 0, 1);
    e3d_Painter_DrawRectangle(ctx, &news_bar, 0x34b2);
    e3d_Painter_DrawRectangle(ctx, &news_bar_top, 0x3292);
    e3d_Painter_Print(ctx, news_text, (int16_t)(-(int32_t)(f * 3u)), 203, 1, 0xffff);
    e3d_Painter_Print(ctx, "Ceny burakuf rosno!!1", 75, 185, 0, 0xffff);
    e3d_Painter_DrawSprite(ctx, assets.mouth[f % 4u], 208, 79, 0, 1);
    if (f <= 15u) {
      e3d_Painter_DrawSprite(ctx, assets.segment_zero, 56, 36, 0, 2);
      e3d_Painter_DrawSprite(ctx, assets.segment_three, 76, 36, 0, 2);
    }
    e3d_Painter_DrawSprite(ctx, assets.logo, 254, 38, 0, 1);
  }
