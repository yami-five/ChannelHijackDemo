...
static void load_assets(e3d_EngineContext *engine_ctx) {
  if (channel != NULL)
    return;
  channel = engine_ctx->storage->get_sprite(SPRITE_CHANNEL3);
  for (uint32_t i = 0; i < CURTAIN_LENGTH; i++)
    curtain[i] = engine_ctx->storage->get_sprite(SPRITE_FIRANKA);
  left_hands[0] = engine_ctx->storage->get_sprite(SPRITE_LEFT4);
  left_hands[1] = engine_ctx->storage->get_sprite(SPRITE_LEFT5);
...
static void run_scene(DemoContext *demo_ctx) {
  ...
  e3d_Painter_DrawSprite(engine_ctx, channel, 91, 76, 0, 1);
  ...
  if (f >= 10u && f < 20u)
    e3d_Painter_DrawSprite(engine_ctx, right_hands[0], rx,
                           (int16_t)(90u - (f - 10u) * 9u), 0, 1);
  else if (f >= 20u && f < 40u)
    e3d_Painter_DrawSprite(engine_ctx, right_hands[1], (int16_t)(rx + 10),
                           wave_offset(f, 350u, 0u, 2), 0, 1);
  else if (f >= 40u && f < 50u)
    e3d_Painter_DrawSprite(engine_ctx, right_hands[0], rx, (int16_t)((f - 40u) * 9u),
                           0, 1);
  else
    e3d_Painter_DrawSprite(engine_ctx, right_hands[0], rx,
...
DemoSceneContext *scene_7_news_out_get_context(void) { return &scene_ctx; }
