...
static void load_assets(e3d_EngineContext *ctx) {
  ...
  hijacking = ctx->storage->get_sprite(64u);
...
static void run_scene(DemoContext *demo_ctx) {
  ...
  if (f >= 10u && f < 20u)
    e3d_Painter_DrawSprite(ctx, right_hands[0], x, (int16_t)(90u - (f - 10u) * 9u), 0, 1);
  else if (f >= 20u && f < 70u)
    e3d_Painter_DrawSprite(ctx, right_hands[1], (int16_t)(x + 10), wave_offset(f, 350u, 0u, 2), 0, 1);
  else if (f >= 70u && f < 80u)
    e3d_Painter_DrawSprite(ctx, right_hands[0], x, (int16_t)((f - 70u) * 9u), 0, 1);
  else
    e3d_Painter_DrawSprite(ctx, right_hands[0], x, (int16_t)(90 + wave_offset(f, 350u, 0u, 2)), 0, 1);
