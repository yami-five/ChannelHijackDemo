...
static void load_assets(e3d_EngineContext *ctx) {
  ...
  for (uint32_t i = 0u; i < 10u; i++)
    digits[i] = ctx->storage->get_sprite((uint8_t)(24u + i));
  tv_bug = ctx->storage->get_sprite(55u);
  logo = ctx->storage->get_sprite(53u);
  background = ctx->storage->get_sprite(56u);
  for (uint32_t i = 0u; i < 4u; i++)
    parts[i] = ctx->storage->get_sprite((uint8_t)(57u + i));
  big_dark = ctx->storage->get_sprite(61u);
  big = ctx->storage->get_sprite(62u);
  break_image = ctx->storage->get_sprite(63u);
}
static void finish(void) {
  scene_ctx.scene_frame++;
  scene_ctx.scene_elapsed_ms = demo_platform_time_ms() - scene_ctx.scene_start_time_ms;
}
static void draw_digits(e3d_EngineContext *ctx, uint8_t left, uint8_t right) {
  e3d_Painter_DrawSprite(ctx, digits[left], 56, 36, 0, 2);
  e3d_Painter_DrawSprite(ctx, digits[right], 76, 36, 0, 2);
}
static void run_scene(DemoContext *demo_ctx) {
  ...
  if (f < 15u) {
    ...
    e3d_Painter_DrawSprite(ctx, logo, 254, 38, 0, 1);
  } else if ((f >= 15u && f < 20u) || (f >= 45u && f < 50u) ||
             (f >= 70u && f < 75u) || (f >= 90u && f < 95u)) {
    e3d_Painter_DrawPlasma(ctx, noise_colors, 4u, f, 2, 3, 4, 5, 3, &plasma_rect);
  } else if (f >= 75u && f < 90u) {
    e3d_Painter_DrawSprite(ctx, break_image, 44, 1, 0, 1);
  } else {
    e3d_Painter_DrawSprite(ctx, background, 44, 1, 0, 1);
    if (f >= 35u) e3d_Painter_DrawSprite(ctx, parts[0], 44, 31, 0, 1);
    if (f >= 65u) e3d_Painter_DrawSprite(ctx, parts[3], 152, 105, 0, 1);
    if (f >= 95u) e3d_Painter_DrawSprite(ctx, parts[2], 44, 121, 0, 1);
    if (f >= 125u) e3d_Painter_DrawSprite(ctx, parts[1], 164, 30, 0, 1);
    e3d_Painter_DrawSprite(ctx, f >= 170u ? big : big_dark, 112,
                           f >= 170u ? 53 : 62, 0, 1);
  }
  if (f >= 100u && f < 115u) draw_digits(ctx, 1u, 0u);
  else if (f >= 90u && f < 100u) draw_digits(ctx, 0u, 9u);
  else if (f >= 75u && f < 90u) draw_digits(ctx, 0u, 8u);
  else if (f >= 60u && f < 75u) draw_digits(ctx, 0u, 7u);
  else if (f >= 45u && f < 60u) draw_digits(ctx, 0u, 6u);
