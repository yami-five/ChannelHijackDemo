...
SUPPORTED_ROTATIONS = (0, 90, 180, -90)
...
def to_c_identifier(value: str) -> str:
    ...
    if not identifier:
        return "texture"
    return re.sub(r"\d+", "texture", identifier)
...
def convert_image_to_rgb565(
    ...
    if rotation not in SUPPORTED_ROTATIONS:
        raise ValueError(
            ...
            f"Choose one of: {', '.join(str(value) for value in SUPPORTED_ROTATIONS)}."
        )

    with Image.open(image_path) as image:
        rgb_image = image.convert("RGB")
        if rotation:
            rgb_image = rgb_image.rotate(rotation, expand=True)
...
def convert_and_save(
    ...
    img_x, img_y, pixels = convert_image_to_rgb565(
        input_path,
        reverse_x=reverse_x,
        rotation=rotation,
    )
...
def run_gui() -> int:
    ...
    class TextureConverterWindow(QMainWindow):
        def __init__(self) -> None:
            ...
            self.rotation_combo.addItem("90° (counter-clockwise)", 90)
            self.rotation_combo.addItem("180°", 180)
            self.rotation_combo.addItem("-90° (clockwise)", -90)
            form_layout.addRow("Rotation:", self.rotation_combo)
            ...
            self.rotation_combo.currentIndexChanged.connect(
                self.try_preview_from_input_path
            )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=0,
        help=(
            "Rotate the exported bitmap by the given number of degrees "
            "counter-clockwise (choices: 0, 90, 180, -90; default: 0)."
        ),
    )
    parser.add_argument(
        "--no-ui",
